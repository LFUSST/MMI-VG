...
class SeqTR(OneStageModel):
    ...
    def forward_train(self,
        ...
        """
        x, y, y_word, y_mask = self.extract_visual_language(
            img, ref_expr_inds)

        x_mm = self.fusion(x, y)

        losses_dict, seq_out_dict = self.head.forward_train(
            x_mm, img_metas, y_word, y_mask,
            gt_bbox=gt_bbox,
    ...
    def forward_test(self,
        ...
        """Args:
            ...
            rescale (bool): whether to rescale predictions from `img_shape`/`pad_shape`
                back to `ori_shape`.
        """
        x, y, y_word, y_mask = self.extract_visual_language(img, ref_expr_inds)
    ...
    def get_predictions(self, seq_out_dict, img_metas, rescale=False):
        ...
        if with_bbox:
            ...
            seq_out_bbox = seq_out_bbox.reshape(batch_size, -1)

    ...
    def __init__(self,
        ...
        if direction == 'bottom_up':
            ...
            self.top_project = nn.Sequential(
                *darknet_conv((ch + vis_chs[-1], ch + vis_chs[-1],), (ch + vis_chs[-1], vis_chs[-1],), (3, 1,),
                              (1, 1,)))
    ...
    def forward(self, x, y):
        """Args
            x (list[tensor]): multi-scale feature maps from visual encoders,
                e.g., for darknet, it's [batch_size, 256, 80, 80], [batch_size, 512, 40, 40],
                [batch_size, 1024, 20, 20] in sequential order.
        ...
        Returns:
            x_multi_modal (tensor): [batch_size, 1024, 20, 20]
        """

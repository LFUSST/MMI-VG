...
def mask_overlaps(gt_mask, pred_masks, is_crowd):
    """Args:
        ...
        pred_mask (list[RLE]):
    """
    def computeOIoU(gt_mask, pred_masks):
        I = np.sum(np.logical_and(decode(gt_mask), decode(pred_masks)))
        U = np.sum(np.logical_or(decode(gt_mask), decode(pred_masks)))
        OIoU = I / U
        return OIoU
    ...
    OIoU = computeOIoU(gt_mask, pred_masks)
    OIoU = torch.from_numpy(np.array(OIoU))
    return OIoU, mask_iou

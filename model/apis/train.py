...
def train_model(epoch,
    ...
    device = list(model.parameters())[0].device       # cuda:0

    batches = len(loader)
    end = time.time()

    det_acc_list, mask_iou_list, mask_acc_list = [], [], []
    loss_det_list, loss_mask_list, loss_multi_task_list = [], [], []
    OIou_list = []
    for batch, inputs in enumerate(loader):
        ...
        loss_multi_task = losses.pop('loss_multi_task', torch.tensor([0.], device=device))
        loss = loss_det + loss_mask + loss_multi_task
        ...
        if cfg.distributed:
            ...
            loss_multi_task = reduce_mean(loss_multi_task)
        ...
        with torch.no_grad():
            ...
            batch_det_acc, batch_OIou, batch_mask_iou, batch_mask_acc_at_thrs = accuracy(
                pred_bboxes, gt_bbox, pred_masks, gt_mask, is_crowd=is_crowd, device=device)

            if cfg.distributed:
                batch_det_acc = reduce_mean(batch_det_acc)
                batch_OIou = reduce_mean(batch_OIou)
        ...
        OIou_list.append(batch_OIou.item())
        ...
        loss_multi_task_list.append(loss_multi_task.item())
        det_acc = sum(det_acc_list) / len(det_acc_list)
        OIou = sum(OIou_list) / len(OIou_list)
        ...
        if is_main():
            if batch % cfg.log_interval == 0 or batch == batches - 1:
                ...
                logger.info(f"train - epoch [{epoch+1}]-[{batch+1}/{batches}] " +
                            ...
                            f"loss_multi_task_list: {sum(loss_multi_task_list) / len(loss_multi_task_list):.4f}, " +
